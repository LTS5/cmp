...
def serial_open(fileobj):
    ...
    if str(hdr['id_string'])[:5] != 'TRACK':
        raise HeaderError('Expecting TRACK as first 5 characters of id_string')
    version = hdr['version']
    if version not in (1, 2):
        raise HeaderError('Reader only supports versions 1 and 2')
    if version == 1:
...
def serial_read(fileobj, hdr):
	n_s = hdr['n_scalars']
	n_p = hdr['n_properties']
	f4dt = np.dtype(native_code + 'f4')
	pt_cols = 3 + n_s
	pt_size = 4 * pt_cols
	ps_size = 4 * n_p
	i_fmt = native_code + 'i'
	...
	xyz = pts[:,:3]
	xyz.flags.writeable = True
	xyz[:,0] = xyz[:,0] / hdr['voxel_size'][0] - 0.5
	xyz[:,1] = xyz[:,1] / hdr['voxel_size'][1] - 0.5
	xyz[:,2] = xyz[:,2] / hdr['voxel_size'][2] - 0.5
	if n_s:
		scalars = pts[:,3:]
	streamlines = (xyz, scalars, ps)
